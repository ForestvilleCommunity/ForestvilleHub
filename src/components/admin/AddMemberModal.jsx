...
export default function AddMemberModal({ team, squads = [], onClose, onSaved }) {
  const [form, setForm] = useState({ name: '', date_of_birth: '', gender: '' });
  const [saving, setSaving] = useState(false);

  const upd = (k, v) => setForm(f => ({ ...f, [k]: v }));

  // Find squad for this team
  const squad = team ? squads.find(s => {
    try { return JSON.parse(s.team_ids || '[]').includes(team.id); } catch { return false; }
  }) : null;
